...
class DataDjpu {
    ...
    public function get_d_djpu($limit = null, $batas = null) {
        ...
        foreach ($result as $val) {
            $d_djpu = new $this($this->registry);
            $d_djpu->set_kd_d_djpu($val['kd_d_djpu']);
            $d_djpu->set_kd_d_user($val['kd_d_user']);
            $d_djpu->set_kd_d_tgl($val['kd_d_tgl']);
            $d_djpu->set_kd_d_node($val['kd_d_node']);
            $d_djpu->set_kd_d_masalah($val['kd_d_masalah']);
    ...
    public function get_d_djpu_by_id($d_djpu = DataDjpu) {
        if (is_null($d_djpu->get_kd_d_djpu())) {
            return false;
        }
        $sql = "SELECT * FROM " . $this->_table . " WHERE kd_d_djpu=" . $d_djpu->get_kd_d_djpu();
//        var_dump($sql);
        $result = $this->db->select($sql);
        foreach ($result as $val) {
            $this->set_kd_d_djpu($val['kd_d_djpu']);
            $this->set_kd_d_user($val['kd_d_user']);
            $this->set_kd_d_tgl($val['kd_d_tgl']);
            $this->set_kd_d_node($val['kd_d_node']);
    ...
    public function add_d_djpu() {
        $data = array(
            'kd_d_user' => $this->get_kd_d_user(),
            'kd_d_tgl' => $this->get_kd_d_tgl(),
            'kd_d_node' => $this->get_kd_d_node(),
    ...
    public function update_d_djpu() {
        ...
        if (!is_array($data))
            return false;
        $where = ' kd_d_djpu=' . $this->get_kd_d_djpu();
    ...
    public function validate() {
        ...
        if ($this->get_kd_d_node() == "" ) {
            $this->_error .= "Node Upload belum diinput!</br>";
            $this->_valid = FALSE;
        }
    ...
    public function set_kd_d_djpu($djpu) {
        $this->_kd_d_djpu = $djpu;
    }
    ...
    public function set_kd_d_node($node) {
        $this->_kd_d_node = $node;
    }
    ...
    public function get_kd_d_djpu($where = null) {
        if (!is_null($where)) {
            $sql = "SELECT kd_d_djpu FROM '" . $this->_table . "' WHERE '" . $where . "'";
            $result = $this->db->select($sql);
            foreach ($result as $val) {
                $this->set_kd_d_djpu($val['kd_d_djpu']);
            }
        }
        return $this->_kd_d_djpu;
    }
    ...
    public function get_kd_d_node() {
        return $this->_kd_d_node;
    }
